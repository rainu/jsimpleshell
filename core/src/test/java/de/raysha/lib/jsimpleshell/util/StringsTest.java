...
public class StringsTest {
	...
	public void testJoinStrings() {
		...
		for (int i = 0; i < cases.length; i++) {
			assertEquals(results[i], Strings.joinStrings(
					Arrays.asList(results[i].split("\\|")), false, '-'));
		}
	}
	...
	public void testSplitJavaIdentifier() {
		...
		for (int i = 0; i < cases.length; i++) {
			assertEquals(dashJoined[i], Strings.joinStrings(
					Strings.splitJavaIdentifier(cases[i]), true, '-'));
		}
	}
